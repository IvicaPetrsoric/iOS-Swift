...
//  youtube
//
//  Created by Ivica OS X on 01/07/17.
...
class VideoPlayerView: UIView{
    ...
    lazy var pausePlayButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(handlePuase), for: .touchUpInside)
        return button
    }()
    
    var isPlaying = false
    
    @objc func handlePuase(){
        if isPlaying{
            player?.pause()
    ...
    override init(frame: CGRect) {
        ...
        videoLenghtLabel.rightAnchor.constraint(equalTo: rightAnchor, constant: -8).isActive = true // padding od desne
    ...
    func setupPlayerViewo(){
        ...
        if let url = URL(string: urlString){
            ...
            let interval = CMTime(value: 1, timescale: 2) // seconds
...
class VideoLauncher: NSObject{
    ...
    func showVideoPlayer(){
        ...
        if let keyWindow = UIApplication.shared.keyWindow{
            ...
            // 16 * 9 aspect radio od svih hd videa
